...
VALIDATION_CASES = [
    {
        ...
        "expected_code": "P0420",
        "expected_keywords": ["catalyst", "catalytic", "converter", "severity"],
        "expected_severity": "warning",
    },
    {
        "prompt": "My car shows P0300. Is it serious?",
        "expected_code": "P0300",
        "expected_keywords": ["misfire", "cylinder", "severity", "do not drive"],
        "expected_severity": "critical",
    },
    {
        "prompt": "What does code P0171 mean?",
        "expected_code": "P0171",
        "expected_keywords": ["lean", "maf", "severity", "causes"],
        "expected_severity": "warning",
    },
    {
        "prompt": "Code P0128 appeared. Should I worry?",
        "expected_code": "P0128",
        "expected_keywords": ["coolant", "thermostat", "severity", "causes"],
        "expected_severity": "warning",
    },
    {
        "prompt": "I have P0562 on my Ford F-150.",
        "expected_code": "P0562",
        "expected_keywords": ["voltage", "battery", "severity", "do not drive"],
        "expected_severity": "critical",
    },
    {
        "prompt": "Code P0087 on my Chevrolet Silverado. What does it mean?",
        "expected_code": "P0087",
        "expected_keywords": ["fuel", "pressure", "severity", "do not drive"],
        "expected_severity": "critical",
    },
    {
        "prompt": "What is P0455?",
        "expected_code": "P0455",
        "expected_keywords": ["evap", "leak", "severity", "gas cap"],
        "expected_severity": "warning",
    },
    {
        "prompt": "My Honda Civic shows P0302. Is it safe to drive?",
        "expected_code": "P0302",
        "expected_keywords": ["misfire", "cylinder", "severity", "do not drive"],
        "expected_severity": "critical",
    },
...
def run_inference(cli: Path, model: Path, adapter: Path, prompt: str, n_predict: int = 150) -> str:
    ...
    adapter == MERGED_MODEL_PATH when generated by Unsloth — used directly as
    the model, no --lora flag.  If a separate LoRA file is passed instead
    (QVAC Fabric output), it is loaded with --lora on top of `model`.
    """
    full_prompt = (
    ...
    is_merged = (adapter == MERGED_MODEL_PATH)

    if is_merged:
        # Unsloth output: use merged file directly, no base model needed
        cmd = [
            str(cli),
            "-m", str(adapter),
            "-ngl", "999",
        ...
        # QVAC Fabric output: separate LoRA adapter on top of base model
        cmd = [
            str(cli),
            "-m", str(model),
            "--lora", str(adapter),
...
def main():
    ...
    if not adapter.exists():
        print(f"\n  [!] Modelo no encontrado en {adapter}")
        print("  [!] Opciones:")
        print("      A) Unsloth/Colab: coloca el .gguf en output/adapter/")
        print("      B) QVAC Fabric:   ejecuta python scripts/04_run_finetune.py")
        return

    is_merged = (adapter.resolve() == MERGED_MODEL_PATH.resolve())
    mode = "Unsloth merged model (no base needed)" if is_merged else "QVAC Fabric LoRA adapter"
    print(f"\n  Modelo:    {adapter}")
    print(f"  Tamaño:    {adapter.stat().st_size / 1024 / 1024:.1f} MB")
    print(f"  Modo:      {mode}")
    ...
    is_merged = (adapter.resolve() == MERGED_MODEL_PATH.resolve())
    if is_merged:
        model = adapter  # merged model IS the model
    else:
        try:
            model = find_model()
        except FileNotFoundError as e:
